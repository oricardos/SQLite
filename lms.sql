...
('José Pereira', hex(randomblob(8)), 'jose@email.com', 'jose'),
...
SELECT * from lessons_completed_full WHERE id = 3;

CREATE VIEW certificates_full as
SELECT cert.id, cert.user_id, u.name, cert.course_id, c.title, c.hours, c.lessons, cert.completed
...
join lessons as l on l.course_id = cl.course_id 
    AND l.`order` BETWEEN cl.`order` - 1 
    AND cl.`order` + 1
ORDER BY l.`order`;
...
SELECT * from certificates_full WHERE id = '898b2fc2509ac182';
...
-- Updates --
UPDATE users set email = 'renata@email.com' WHERE id = 9;
UPDATE users set password_hash = '1234567890' where id = 9;
UPDATE sessions SET expires = strftime('%s', 'now', '+15 days') where token = '4AD836B671FA487E';
